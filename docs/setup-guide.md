...
| Homepage URL | `https://oosawak.github.io/OreCopilot/` |
| Authorization callback URL | 同じ |

3. **Client ID** と **Client Secret** をコピー → GitHub Actions Secrets に登録

---

### Step 4: Cloudflare Worker のデプロイ
...
| `ALLOWED_ORIGIN` | `https://oosawak.github.io/OreCopilot/` | Plain text |

Worker の URL をメモしておく（例: `https://second-brain-oauth.xxx.workers.dev`）

---

### Step 5: GitHub Actions Secrets の設定

GitHub リポジトリ → **Settings** → **Secrets and variables** → **Actions** で以下を登録：

| Secret名 | 値 |
|----------|-----|
| `VITE_GITHUB_CLIENT_ID` | Step 3 の Client ID |
| `VITE_OAUTH_WORKER_URL` | Step 4 の Worker URL |

---

### Step 6: リモートリポジトリにプッシュ

```bash
git remote add origin https://github.com/oosawak/OreCopilot.git
git branch -M master
...
GitHub Actions が自動的に `master` ブランチの push を検知してビルド・デプロイします。

---

### Step 8: アクセス確認

デプロイ完了後（Actions ワークフロー完了後）、以下にアクセス：

```
https://oosawak.github.io/OreCopilot/
```

「GitHubでログイン」ボタンをクリック → GitHub 認証ページへリダイレクト → OAuth 完了 → アプリ利用開始
